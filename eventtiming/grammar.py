...
def geo_dist(n_iti, total_iti_dur, mn=0.5, mx=6, seed=None):
    ...
    if n_iti * mx < total_iti_dur:
        ...
        warn(
            f"total iti duration {total_iti_dur}s too large to fill with {mx}s*{n_iti}trials"
        )
    ...
    while sum(itis) < total_iti_dur - mn / 2:
        ...
        if not small_enough:
            warn(
                f"ITI durations cannot backfill. have {sum(itis)} < {total_iti_dur} total. but no itis < {mx - mn}"
            )
            break
    ...
    goldilocks = [i for i in range(len(itis)) if itis[i] + mn < mx and itis[i] > mn * 2]
    if goldilocks:
